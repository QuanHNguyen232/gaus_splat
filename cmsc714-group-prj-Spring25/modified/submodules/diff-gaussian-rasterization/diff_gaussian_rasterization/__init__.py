...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        bg = raster_settings.bg.to(device)
        viewmatrix = raster_settings.viewmatrix.to(device)
        projmatrix = raster_settings.projmatrix.to(device)
        campos = raster_settings.campos.to(device)
    ...
    def backward(ctx, grad_out_color, _, grad_out_depth):
        ...
        if grad_out_depth is not None:
            grad_out_depth = grad_out_depth.to(device)
...
class GaussianRasterizer(nn.Module):
    ...
    def markVisible(self, positions):
        ...
        with torch.no_grad():
            ...
            visible = _C.mark_visible(
                positions,
                raster_settings.viewmatrix.to(device),
                raster_settings.projmatrix.to(device))
    ...
    def forward(self, means3D, means2D, opacities, shs = None, colors_precomp = None, scales = None, rotations = None, cov3D_precomp = None):
        ...
        cov3D_precomp = cov3D_precomp.to(device)
